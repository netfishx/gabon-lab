...
class JwtService(val config: JwtConfig) {
    ...
    private val customerVerifier = JWT.require(customerAlgorithm)
        .withIssuer(CUSTOMER_ISSUER)
        .withAudience(CUSTOMER_AUDIENCE)
        .build()

    private val adminVerifier = JWT.require(adminAlgorithm)
    ...
    fun parseCustomerToken(token: String): TokenClaims =
        parseToken(token, customerVerifier)

    fun parseAdminToken(token: String): TokenClaims =
        parseToken(token, adminVerifier)

    private fun parseToken(token: String, verifier: com.auth0.jwt.JWTVerifier): TokenClaims {
        val payload = try {
