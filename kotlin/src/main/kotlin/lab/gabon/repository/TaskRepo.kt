...
class TaskRepo {
    ...
    suspend fun findActiveTaskDefinitions(taskType: Short? = null): List<TaskDefinitionRow> = dbQuery {
        TaskDefinitions.selectAll()
            .where {
    ...
    suspend fun findProgressById(progressId: Long): TaskProgressRow? = dbQuery {
        TaskProgress.selectAll()
    ...
    ): List<TaskWithProgressRow> = dbQuery {
        TaskDefinitions
            .join(TaskProgress, JoinType.INNER, TaskDefinitions.id, TaskProgress.taskId)
            .selectAll()
            .where {
                var cond = (TaskProgress.customerId eq customerId) and
    ...
    suspend fun incrementProgress(progressId: Long): Unit = dbQuery {
    ...
    suspend fun claimReward(progressId: Long, customerId: Long): Int = dbQuery {
        val jdbcConn = TransactionManager.current().connection.connection as java.sql.Connection
        ...
        jdbcConn.prepareStatement(lockSql).use { stmt ->
            ...
            if (rs.next()) {
                ...
                if (status != TaskStatus.COMPLETED.value) {
                    val msg = if (status == TaskStatus.CLAIMED.value) "task already claimed"
    ...
    private fun ResultRow.toTaskDefinitionRow(): TaskDefinitionRow = TaskDefinitionRow(
    ...
    private fun ResultRow.toTaskProgressRow(): TaskProgressRow = TaskProgressRow(
