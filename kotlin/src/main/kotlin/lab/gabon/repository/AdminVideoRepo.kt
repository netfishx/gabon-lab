...
class AdminVideoRepo {
    ...
    suspend fun listVideosAdmin(
        ...
        endDate: String? = null,
        isVip: Boolean? = null,
    ): Pair<List<AdminVideoListRow>, Long> = dbQuery {
        ...
        if (endDate != null) {
            ...
            val end = LocalDate.parse(endDate).plusDays(1).atStartOfDay().atOffset(ZoneOffset.UTC)
    ...
    suspend fun getVideoDetailAdmin(videoId: Long): AdminVideoRow? = dbQuery {
    ...
    suspend fun adminDeleteVideo(videoId: Long): Boolean = dbQuery {
        Videos.update({
            (Videos.id eq videoId) and Videos.deletedAt.isNull()
        }) {
    ...
    suspend fun listCustomers(
        ...
        name: String? = null,
        isVip: Boolean? = null,
    ): Pair<List<CustomerRow>, Long> = dbQuery {
    ...
    private fun ResultRow.toAdminVideoRow(): AdminVideoRow = AdminVideoRow(
    ...
    private fun ResultRow.toAdminVideoListRow(): AdminVideoListRow = AdminVideoListRow(
        id = this[Videos.id].value,
        customerId = this[Videos.customerId].value,
        title = this[Videos.title],
        fileName = this[Videos.fileName],
