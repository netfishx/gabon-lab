...
class LikeRoutesTest {
    ...
    private fun aliceToken(): String =
        jwtService.generateCustomerTokens(1L).accessToken
    ...
    @Test
    fun `like approved video - 200`() = testApplication {
        setupApp()
    ...
    @Test
    fun `like non-approved video - 403 VIDEO_NOT_APPROVED`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { videoRepo.findById(99) } returns makeVideoRow(
            id = 99,
            status = VideoStatus.PENDING_REVIEW.value,
        )

        val client = jsonClient()
        val response = client.post("/api/v1/videos/99/like") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `like nonexistent video - 404 VIDEO_NOT_FOUND`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { videoRepo.findById(999999) } returns null

        val client = jsonClient()
        val response = client.post("/api/v1/videos/999999/like") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `like already-liked video - idempotent 200, repo returns false`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `like without auth - 401`() = testApplication {
        setupApp()
    ...
    @Test
    fun `unlike previously liked video - 200`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `unlike not-liked video - idempotent 200`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `unlike without auth - 401`() = testApplication {
        setupApp()
