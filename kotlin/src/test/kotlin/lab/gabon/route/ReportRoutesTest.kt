...
class ReportRoutesTest {
    ...
    @Test
    fun `revenue report - returns date-grouped claim counts and diamond sums`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        coEvery { reportRepo.revenueReport("2026-03-01", "2026-03-19") } returns listOf(
        ...
        val response = client.get("/admin/v1/reports/revenue?start_date=2026-03-01&end_date=2026-03-19") {
    ...
    @Test
    fun `video daily report - returns date-grouped upload and engagement counts`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        coEvery { reportRepo.videoDailyReport("2026-03-01", "2026-03-19") } returns listOf(
            VideoDailyReportRow(
                date = "2026-03-15",
                uploadCount = 10,
                totalClicks = 500,
        ...
        val client = jsonClient()
        val response = client.get("/admin/v1/reports/video/daily?start_date=2026-03-01&end_date=2026-03-19") {
            bearerAuth(adminToken())
        }
    ...
    @Test
    fun `video summary report - returns aggregate totals with status breakdowns`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        coEvery { reportRepo.videoSummaryReport("2026-03-01", "2026-03-19") } returns VideoSummaryReportRow(
            totalVideos = 100,
            approvedCount = 60,
            pendingCount = 25,
            rejectedCount = 15,
        ...
        val client = jsonClient()
        val response = client.get("/admin/v1/reports/video/summary?start_date=2026-03-01&end_date=2026-03-19") {
            bearerAuth(adminToken())
        }
