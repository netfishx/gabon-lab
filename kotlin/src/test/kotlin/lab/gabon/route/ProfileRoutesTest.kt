...
class ProfileRoutesTest {
    ...
    @Test
    fun `get my profile - 200 with all private fields`() = testApplication {
        setupApp()
    ...
    @Test
    fun `get my profile - 401 without auth`() = testApplication {
        setupApp()
    ...
    @Test
    fun `update profile - 200 partial update`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        ...
        val client = jsonClient()
        val response = client.put("/api/v1/users/me/profile") {
    ...
    @Test
    fun `update profile - empty fields do not overwrite`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `presign avatar upload - 200 with upload and avatar URLs`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        val client = jsonClient()
        val response = client.post("/api/v1/users/me/avatar/upload-url") {
            contentType(ContentType.Application.Json)
            bearerAuth(aliceToken())
    ...
    @Test
    fun `confirm avatar upload - 200`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { customerRepo.updateAvatarUrl(1L, any()) } just Runs

        val client = jsonClient()
        val response = client.post("/api/v1/users/me/avatar/confirm") {
            contentType(ContentType.Application.Json)
            bearerAuth(aliceToken())
    ...
    @Test
    fun `confirm avatar upload - 401 without auth`() = testApplication {
        setupApp()
