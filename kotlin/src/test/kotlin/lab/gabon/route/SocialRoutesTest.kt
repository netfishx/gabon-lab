...
class SocialRoutesTest {
    ...
    private fun customerRow(id: Long, username: String, name: String? = null) = CustomerRow(
        id = id,
        username = username,
        passwordHash = "hash",
        name = name,
    ...
    @Test
    fun `follow another user - 200`() = testApplication {
        setupApp()
        ...
        val response = client.post("/api/v1/users/2/follow") {
    ...
    @Test
    fun `follow nonexistent user - 404 NOT_FOUND`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { customerRepo.findById(999999L) } returns null

        val client = jsonClient()
        val response = client.post("/api/v1/users/999999/follow") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `follow self - 400 USER_CANNOT_FOLLOW_SELF`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        val client = jsonClient()
        val response = client.post("/api/v1/users/1/follow") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `follow already followed user - 409 USER_ALREADY_FOLLOWING`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { customerRepo.findById(2L) } returns customerRow(2, "bob")
    ...
    @Test
    fun `unfollow a user - 200`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { socialRepo.unfollow(1L, 2L) } returns true

        val client = jsonClient()
        val response = client.delete("/api/v1/users/2/follow") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `unfollow self - 400 USER_CANNOT_FOLLOW_SELF`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false

        val client = jsonClient()
        val response = client.delete("/api/v1/users/1/follow") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `unfollow not following - 400 USER_NOT_FOLLOWING`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `one-way follow status = 1`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
    ...
    @Test
    fun `mutual follow status = 2`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { customerRepo.findById(2L) } returns customerRow(2, "bob", "Bob")
    ...
    @Test
    fun `no follow status = 0`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        coEvery { customerRepo.findById(2L) } returns customerRow(2, "bob", "Bob")
    ...
    @Test
    fun `unauthenticated viewer sees follow_status = 0`() = testApplication {
        setupApp()
        coEvery { customerRepo.findById(2L) } returns customerRow(2, "bob", "Bob")
    ...
    @Test
    fun `viewing own profile shows follow_status = 0`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        ...
        val client = jsonClient()
        val response = client.get("/api/v1/users/1") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `get my following list`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        val items = listOf(
        ...
        val client = jsonClient()
        val response = client.get("/api/v1/users/me/following?page=1&page_size=20") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `get my followers list`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        val items = listOf(
        ...
        val client = jsonClient()
        val response = client.get("/api/v1/users/me/followers?page=1&page_size=20") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `following list shows mutual status`() = testApplication {
        setupApp()
        coEvery { tokenStore.isBlacklisted(any()) } returns false
        val items = listOf(
        ...
        val client = jsonClient()
        val response = client.get("/api/v1/users/me/following") {
            bearerAuth(aliceToken())
        }
    ...
    @Test
    fun `get another user's following list (public)`() = testApplication {
        setupApp()
        val items = (1..5).map {
    ...
    @Test
    fun `get another user's followers list (public)`() = testApplication {
        setupApp()
        val items = (1..3).map {
            FollowUserRow(it.toLong() + 10, "user$it", "User $it", null, 0)
        }
    ...
    @Test
    fun `get public profile - 200 with counts and follow_status`() = testApplication {
        setupApp()
        coEvery { customerRepo.findById(2L) } returns customerRow(2, "bob", "Bob")
        coEvery { socialRepo.getFollowingCount(2L) } returns 10
        coEvery { socialRepo.getFollowerCount(2L) } returns 5
    ...
    @Test
    fun `get nonexistent user profile - 404`() = testApplication {
        setupApp()
        coEvery { customerRepo.findById(999999L) } returns null
