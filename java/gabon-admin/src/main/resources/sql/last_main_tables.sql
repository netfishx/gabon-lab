...
(
    ...
    comment '广告商表' collate = utf8mb4_unicode_ci;

create index idx_deleted_flag
    on advertiser (deleted_flag);
...
create index idx_advertiser_id
    on advertisement (advertiser_id);

create index idx_deleted_flag
    on advertisement (deleted_flag);

create index idx_remain_count
    on advertisement (remain_count);

create index idx_status
    on advertisement (status);
